...
class deep_auto_encoder(object):
    ...
    def init_sigmoid_layers(self):
        ...
        if self.pre != None:
            pre_mat_file = '%s/DAE_%s.mat'%(self.output_folder, self.pre)
...
#             set_trace()
    ...
    def get_model_file(self):
        ...
        if self.p != 0:
            ...
            file_str = ''
        if self.pre !=None:
            file_str = '%s_(%s)' % (file_str, self.pre)
...
def train_DAE(datasets, layers_sizes, output_folder, p, sigma, pre=None):
    ...
    logger = mylogger('%s/log2_p%g_s%g.log' % (output_folder, p, sigma))
    logger.log('train_set size: %d'%(n_train_batches*batch_size))
    logger.log('layer size: ' + ''.join(['%d ' % i for i in layers_sizes]).strip())
    rng = numpy.random.RandomState(123)
    dae = deep_auto_encoder(numpy_rng=rng, layers_sizes=layers_sizes, output_folder=output_folder, p=p, sigma=sigma, pre=pre, logger=logger)
...
#     layers_sizes = [input_dim,500,54,500,input_dim]
    for p in p_list:
        for sigma in sigma_list:
            train_DAE(datasets, layers_sizes, output_folder, p, sigma, 'p0_s0')            
